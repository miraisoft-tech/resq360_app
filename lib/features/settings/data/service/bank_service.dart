...
class BankRepo extends BaseAPI {
  ...
  Future<ApiResult<BankDetails>> addBankAccount({
    ...
    try {
      ...
      if (res.statusCode == 201 && res.data != null) {
        final data = BankDetails.fromJson(
        ...
        return ApiResult(
          error:
              res.data?['message'].toString() ?? 'Failed to add bank account',
        );
      }
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        final bankAccountsData =
            (json['data'] as List)
                .map(
                  (item) => BankDetails.fromJson(item as Map<String, dynamic>),
                )
                .toList();
  ...
  Future<ApiResult<String>> verifyAndRegisterABankAccountWithPaystack(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ApiResult(data: data.toString());
      } else {
  ...
  Future<ApiResult<Map<String, dynamic>>> validateBankAccount({
    ...
    try {
      ...
      if (res.statusCode == 201 && res.data != null) {
        ...
        return ApiResult(
          error:
              res.data?['message']?.toString() ?? 'Failed to validate account',
        );
      }
    } on Exception catch (e) {
      return ApiResult(error: e.toString());
  ...
  Future<ApiResult<List<BankModel>>> fetchBanksFromServer() async {
    const url = '/bank-account/banks';
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        return ApiResult(data: banks);
      } else {
        return ApiResult(
          error:
              response.data?['message']?.toString() ?? 'Failed to fetch banks',
        );
      }
    } on DioException catch (e) {
      return handleDioError(e);
